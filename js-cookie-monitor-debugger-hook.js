...
GM_config.init({
    id: "js-cookie-monitor",
    title: "JS Cookie Monitor/Debugger Hook",
    fields: {
        "logFontSize": {
            "label": "日志字体大小",
            "title": "在控制台打印日志时字体大小，根据自己喜好调整",
            "type": "float",
            "default": 12
        },
        "ignoreUnchanged": {
            "label": "忽略不变的cookie",
            "title": "使用document.cookie更新cookie，但是cookie新的值和原来的值一样，此时要不要忽略这个事件",
            "type": "checkbox",
            "default": true
        },
        "showDateTime": {
        ...
        "EventDebugger.rules": {
            "label": "规则（一行一个，/表示使用正则/）",
            "section": ['EventDebugger', '设置事件断点是否开启，一般保持默认即可'],
            "type": "textarea",
            "default": ""
        },
        "EventDebugger.add": {
...
function init() {
    ...
    const enableEventDebugger = {
        "add": GM_config.get("EventDebugger.add"),
        "update": GM_config.get("EventDebugger.update"),
        "delete": GM_config.get("EventDebugger.delete"),
        "read": GM_config.get("EventDebugger.read")
    }

    // 在控制台打印日志时字体大小，根据自己喜好调整
    // 众所周知，12px是宇宙通用大小
    const consoleLogFontSize = GM_config.get("logFontSize");

    const consoleLogDateTime = GM_config.get("showDateTime");

    // 使用document.cookie更新cookie，但是cookie新的值和原来的值一样，此时要不要忽略这个事件
    const ignoreUpdateButNotChanged = GM_config.get("ignoreUnchanged");
